...
function WeaknessCard({ weakness, variant, onAction }: { weakness: WeaknessData, variant: 'critical' | 'attention' | 'improving', onAction: () => void }) {
  ...
  const subjectColors = { PHYSICS: "bg-blue-50 text-blue-600", CHEMISTRY: "bg-teal-50 text-teal-600", BIOLOGY: "bg-amber-50 text-amber-600" }
  const TrendIcon = weakness.trend === 'declining' ? TrendingDown : weakness.trend === 'improving' ? TrendingUp : Minus

  return (
    <Card className={cn("min-w-[340px] border-none shadow-sm rounded-3xl overflow-hidden bg-white transition-all hover:shadow-md group", variant === 'critical' ? "w-[340px]" : "w-full")}>
      <CardContent className="p-0 flex flex-col h-full">
        <div className={cn("h-1 w-full", colors[variant].accent)} />
        <div className="p-6 space-y-6 flex-1">
          <div className="flex justify-between items-start">
            <div className="space-y-1">
              <Badge className={cn("border-none font-bold text-[9px] uppercase px-2 py-0", subjectColors[weakness.subject])}>{weakness.subject}</Badge>
              <h3 className="text-lg font-bold text-slate-900 group-hover:text-primary transition-colors tracking-tight truncate">{weakness.chapterName}</h3>
            </div>
            <div className={cn("p-2 rounded-xl", colors[variant].bg)}><TrendIcon className={cn("w-4 h-4", colors[variant].text)} /></div>
          </div>
          <div className="space-y-3">
            ...
            <Progress value={weakness.accuracy} className={cn("h-1.5", colors[variant].accent)} />
          </div>
          <div className="grid grid-cols-2 gap-4 pt-2">
            <div className="space-y-1">
              <p className="text-[9px] font-bold text-slate-400 uppercase">Attempts</p>
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700"><Activity className="w-3 h-3 text-slate-300" />{weakness.totalAttempts} Sessions</div>
            </div>
            <div className="space-y-1">
              <p className="text-[9px] font-bold text-slate-400 uppercase">Last Activity</p>
              <div className="flex items-center gap-1.5 text-xs font-bold text-slate-700"><Clock className="w-3 h-3 text-slate-300" />{weakness.lastAttempted}</div>
            </div>
          </div>
          {weakness.dependentChapters > 0 && (
            <div className="p-3 bg-slate-50 rounded-2xl flex items-center gap-3 border border-slate-100">
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tight">Blocks <span className="text-primary">{weakness.dependentChapters} Related Topics</span></p>
            </div>
          )}
        </div>
        <div className="p-4 bg-slate-50/50 border-t border-slate-100">
          <Button onClick={onAction} className="w-full rounded-xl h-11 font-bold text-xs gap-2 group/btn shadow-none" variant="outline">
